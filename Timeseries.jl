...
serie = convert(Array{Float64,1},dados[!,"315887"])
...
for q in q_candidates
    for d in d_candidates
        model = arima.ARIMA(serie,order=(q,0,d)).fit() 
...
selection = Dict("Resultados"=>modelo,"Aic"=>results_aic,"Bic"=>results_bic)

# 4-3 Estimando o melhor modelo 
# 4-3-1 Aic
best_model_aic = arima.ARIMA(serie,order=(2,0,0)).fit()
best_model_aic.summary()

# 4-3-2 Bic 
best_model_bic = arima.ARIMA(serie,order=(0,0,1)).fit()
best_model_bic.summary()

# 5 Rmse 
best_model_rmse= arima.ARIMA(serie,order=(2,0,0)).fit()
